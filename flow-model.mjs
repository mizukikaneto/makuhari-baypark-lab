import {arenaRoutes} from './map-data.mjs?v=access2';
export const FLOW_DEFAULTS={direction:'depart',people:8000,releaseMinutes:15,parkShare:70,width:4,gates:2};
export const FLOW_FIELDS={gates:{type:'integer',enum:[1,2,4]},direction:{type:'string',enum:['arrive','depart']},people:{type:'integer',minimum:1000,maximum:20000,multipleOf:1000},releaseMinutes:{type:'integer',minimum:5,maximum:60,multipleOf:5},parkShare:{type:'integer',minimum:0,maximum:100,multipleOf:10},width:{type:'integer',minimum:2,maximum:8}};
...
export function createFlowModel(options={}){
  validateFlow(options);const config={...FLOW_DEFAULTS,...options},base=arenaRoutes(config.gates);
  ...
  const assigned={park:0,south:0};
  const packets=Array.from({length:config.people/weight},(_,i)=>{
    const group=Math.floor((i+1)*config.parkShare/100)>Math.floor(i*config.parkShare/100)?'park':'south';
    const candidates=Object.keys(routes).filter(id=>id.startsWith(group+'-'));
    const routeId=candidates[assigned[group]++%candidates.length];
  ...
  function snapshot(){
    ...
    const byGate=Object.values(routes).reduce((a,r)=>{if(!a[r.gateId]){const people=packets.filter(p=>routes[p.routeId].gateId===r.gateId);a[r.gateId]={name:r.gateName,total:people.length*weight,arrived:people.filter(p=>p.done).length*weight};}return a;},{});
    return {byGate,time,total:config.people,arrived,walking,waiting,pending,queue,peakQueue,averageMinutes:arrived?journeyTotal/arrived/60:null,complete:arrived===config.people,busiest:busy?{key:busy.key,people:busy.count,density:busy.count/busy.area}:null};
  }
