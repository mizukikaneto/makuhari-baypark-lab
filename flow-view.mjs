...
import {EXITS} from './map-data.mjs?v=access2';
export function createFlowView(root,lineGeometry,addLabel){
  ...
  function set(next){
    ...
    for(const l of labels)l.f.active=false;
    const seen=new Set();for(const r of Object.values(model.routes)){
      if(!seen.has(r.stationAccess)){const p=EXITS[r.stationAccess].point;marker(p,r.stationGroup==='park'?0x85f7cf:0x8dbaff);putLabel('flow-'+r.stationAccess,EXITS[r.stationAccess].name,p);seen.add(r.stationAccess);}
      if(!seen.has(r.gateId)){marker(r.endpoint,0xf5bf89);putLabel('flow-'+r.gateId,r.gateName+'（仮定）',r.endpoint);seen.add(r.gateId);}
    }
  }
  ...
  function putLabel(id,name,p){let l=labels.find(l=>l.f.id===id);if(!l){l=addLabel({id,short:name,x:p[0],z:p[1],year:1986,type:'flow'},24,'flow-point');labels.push(l);}else l.p.set(p[0],24,p[1]);l.f.active=true;}
  function draw(){
    ...
    people.forEach((p,i)=>{const offset=((p.id%5)-2)*.75;dummy.position.set(p.x-Math.sin(p.angle)*offset,10,p.z+Math.cos(p.angle)*offset);dummy.scale.setScalar(p.blocked?1.35:1);dummy.updateMatrix();dots.setMatrixAt(i,dummy.matrix);color.set(p.blocked?0xffb28e:p.routeId.startsWith('park-')?0xc4ffe7:0xa9cfff);dots.setColorAt(i,color);});
